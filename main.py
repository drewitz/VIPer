...
def main_tesselation():
    ...
    plt.savefig("tesselation.png", bbox_inches='tight', pad_inches = 0, transparent=True)
    plt.show()
...
def subgroup_tesselation():
    ...
    new_from_inverse = [g.new_from_matrix(A) for A in inv_gens for g in start_geods]
    for i in range(1):
...
def disc():
    a = dgd.DiscGeodesic(np.pi/4, np.pi, 'r')
    b = dgd.DiscGeodesic(np.pi/5, np.pi, 'g')
    c = dgd.DiscGeodesic(np.pi/2, np.pi*3/2, 'k')
